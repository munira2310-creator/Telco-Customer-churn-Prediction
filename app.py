...
satisfaction_col = 'Satisfaction Score'
tenure_col = 'Tenure in Months'
referrals_col = 'Number of Referrals'
...
with tab1:
    ...
    if st.button("Run Prediction"):
        # The model expects feature names as trained (usually with underscores)
        # We ensure the input DataFrame matches the 'features' list exactly
        input_data = pd.DataFrame(0, index=[0], columns=features)
        
        # Mapping sliders to model features (assuming model uses these names)
        # If your model uses 'Satisfaction Score' with space, change underscores to spaces
        ...
        if prediction[0] == 1 or str(prediction[0]) == 'True' or str(prediction[0]) == 'Yes':
...
with tab2:
    ...
    # Check if our target columns exist in the dataframe
    if churn_col in df.columns and charge_col in df.columns:
        ...
        st.error("Column naming mismatch. Please check CSV headers.")
